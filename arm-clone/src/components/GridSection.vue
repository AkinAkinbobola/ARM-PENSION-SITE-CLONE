<template>
  <div class="bg-cream pb-12 flex flex-col md:flex-row md:py-14 md:px-20 p-6 justify-center">
    <div class="grid grid-cols-3 grid-rows-2 gap-4">
      <img src="../assets/images/grid/grid2.jpg" class="w-20 col-span-2" alt="Image 2">
      <img src="../assets/images/grid/grid3.jpg" class="w-20" alt="Image 3">
      <img src="../assets/images/grid/grid5.jpg" class="w-20" alt="Image 5">
      <img src="../assets/images/grid/grid4.jpg" class="w-20" alt="Image 4">
      <img src="../assets/images/grid/grid1.jpg" class="w-20" alt="Image 1">
    </div>

    <div class="md:ml-10 flex flex-col md:w-6/12">
      <h1 class="text-blueText text-4xl font-serif mb-6 font-bold">
        Tomorrow is looking good.
      </h1>
      <p class="text-lg text-justify">Retirement is supposed to be enjoyable and stress-free, so we created our planning
        experience to be that way too.
      </p>
      <p class="text-lg mt-6 mb-6 text-justify">This isn't a typical retirement planning experience. It's delightful,
        research-backed,
        and focuses on what's most important: you.
      </p>
      <button class="py-2 px-8 text-white bg-pink-800 ml-0 font-sans rounded w-52">Make it happen</button>
    </div>
  </div>
</template>